...
if [[ "$MAKEVAR_FREEZE_UPDATE" != 1 ]]; then
    ...
    if ! curl github.com --connect-timeout 5 -s > /dev/null; then
        ...
        echo -e "GitHub is not reachable, cannot check for updates. Please check your internet connection."
        echo -n -e "${C_RST}"
        ...
        catapult_update () {
            ...
            if [[ "${MAKEVAR_HOST_USER_ID}" == 1000 ]]; then

                if [[ ${MAKEVAR_CATAPULT_VERSION} == "staging" ]]; then
                    UPDATE_MAKEVAR_IMAGE_TAG=${MAKEVAR_CATAPULT_VERSION}
                else
                    UPDATE_MAKEVAR_IMAGE_TAG=${REMOTE_VERSION}
                fi

                echo -e -n "${C_YELLOW}"
                echo -e "Updating Catapult Docker image..."
                echo -e -n "${C_RST}"
                ${MAKEVAR_SUDO_COMMAND} docker --context default pull "${MAKEVAR_CONTAINER_REGISTRY}/${MAKEVAR_IMAGE_NAME}:${UPDATE_MAKEVAR_IMAGE_TAG}"
