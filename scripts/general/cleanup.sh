...
if ${MAKEVAR_SUDO_COMMAND} docker images | grep -q -E "${MAKEVAR_CONTAINER_REGISTRY}/${MAKEVAR_IMAGE_NAME}|${MAKEVAR_CATAPULT_VERSION}"; then

    echo -e "Deleting ${MAKEVAR_CONTAINER_REGISTRY}/${MAKEVAR_IMAGE_NAME} image..."
    ${MAKEVAR_SUDO_COMMAND} docker rmi -f "${MAKEVAR_CONTAINER_REGISTRY}/${MAKEVAR_IMAGE_NAME}:${MAKEVAR_CATAPULT_VERSION}"
